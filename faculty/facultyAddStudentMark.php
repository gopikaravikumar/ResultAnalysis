...
$qryCheck = "SELECT * FROM `studentresult` WHERE `sid`='$sid' AND `eid`='$eid'";
$resultCheck = mysqli_query($conn, $qryCheck);
if (mysqli_num_rows($resultCheck) > 0) {
    echo "<script>alert('You have already added mark for this exam..');</script>";
    echo "<script>window.location.href='facultyViewStudentofExam.php?eid=$eid';</script>";
}
?>
<main id="main">
...
if (isset($_POST['submit'])) {
    ...
    while ($i <= $countSub) {
        ...
        if (isset($_POST['res' . $i])) {
            $res = $_POST['res' . $i];
            $qry = "INSERT INTO `studentresult`(`sid`, `eid`, `subid`, `result`,`examtype`) VALUES ('$sid','$eid','$subid','$res','$etype')";
            $result = mysqli_query($conn, $qry);
        }
        $i++;
    }
